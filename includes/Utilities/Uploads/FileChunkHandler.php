...
abstract class FileChunkHandler {
	...
	public function ajax_remove_file() {
		...
		$default_error = esc_html__( 'Something went wrong while removing the file.', 'wp-vimeo-videos-pro' );
	...
	public function ajax_upload_init() {
		...
		$default_error = esc_html__( 'Something went wrong, please try again.', 'wp-vimeo-videos-pro' );
	...
	protected function validate_file_upload_error( $error ) {
		...
		if ( array_key_exists( $error, $this->file_upload_errors ) ) {
			return sprintf( esc_html__( 'File upload error. %s', 'wp-vimeo-videos-pro' ), $this->file_upload_errors[ $error ] );
		}
