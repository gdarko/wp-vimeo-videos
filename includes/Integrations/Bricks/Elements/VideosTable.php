...
class VideosTable extends \Bricks\Element {
	...
	public function set_controls() {
		...
		$this->controls['category'] = [
			'tab'         => 'content',
			'group'       => 'query',
			'label'       => esc_html__( 'Category', 'wp-vimeo-videos' ),
			'type'        => 'select',
			'options'     => $options,
			'inline'      => false,
			'clearable'   => false,
	...
	public function render() {
		...
		$params = [
			...
			'category'        => ! empty( $this->settings['category'] ) && is_numeric( $this->settings['category'] ) && (int) $this->settings['category'] >= 1 ? (int) $this->settings['category'] : null,
			...
			'show_pagination' => isset( $this->settings['show_pagination'] ) ? (bool) $this->settings['posts_per_page'] : false,
		];
