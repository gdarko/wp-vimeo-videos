	...
	<?php if ( $plugin->system()->vimeo()->is_connected && $plugin->system()->vimeo()->can_upload() ): ?>
        ...
        <div class="wvv-box" style="max-width: 500px;">
            <form class="wvv-video-upload" enctype="multipart/form-data" method="post" action="/">
                <div class="form-row">
                    <label for="vimeo_title"><?php _e( 'Title', 'vimeify' ); ?></label>
                    <input type="text" name="vimeo_title" id="vimeo_title">
                </div>
                <div class="form-row">
                    <label for="vimeo_description"><?php _e( 'Description', 'vimeify' ); ?></label>
				...
				<?php if ( $enable_view_privacy ): ?>
					...
					?>
                    <div class="form-row">
                        <label for="vimeo_view_privacy"><?php _e( 'View Privacy', 'vimeify' ); ?></label>
				...
				<?php if ( $enable_folders ): ?>
                    <div class="form-row">
                        ...
                        <select id="folder_uri" name="folder_uri" class="dgv-select2" data-action="dgv_folder_search"
                                data-placeholder="<?php _e( 'Select folder...', 'vimeify' ); ?>">
                            <option value="default" <?php selected( 'default', $default_folder_uri ); ?>><?php _e( 'Default (no folder)', 'vimeify' ); ?></option>
				...
				<?php endif; ?>
                <div class="form-row">
                    <label for="vimeo_video"><?php _e( 'Video File', 'vimeify' ); ?></label>
                ...
                <div class="form-row with-border">
                    ...
                    <button type="submit" class="button-primary" name="vimeo_upload" value="1">
						<?php _e( 'Upload', 'vimeify' ); ?>
                    </button>
                </div>
            </form>
        </div>
        <p>
            <a href="<?php echo admin_url( 'admin.php?page=' . \Vimeify\Core\Backend\Ui::PAGE_VIMEO ); ?>"><?php _e( '< Back to library', 'vimeify' ); ?></a>
        </p>
