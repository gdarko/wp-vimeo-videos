...
function wvv_render_media_library_upload_buttons( $id ) {
	...
	if ( ! $is_video ) {
		$output .= "<p>" . __( 'Not a video', 'vimeify' ) . "</p>";
	} else {
		...
		if ( ! isset( $data['vimeo_id'] ) ) {
			...
			if ( current_user_can( 'delete_posts' ) && $api->can_delete() ) {
				$delete = '<a href="#" class="button-primary dgv-delete-attachment" data-id="' . $id . '">' . __( 'Delete from Vimeo', 'vimeify' ) . '</a>';
			}
			$link   = '<a target="_blank" class="button" href="' . $link . '">' . __( 'Vimeo Link', 'vimeify' ) . '</a>';
			$output .= '<p>' . __( 'Video uploaded to Vimeo.', 'vimeify' ) . '</p>';
